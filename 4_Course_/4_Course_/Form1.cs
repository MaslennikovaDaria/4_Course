...
namespace _4_Course_
{
    public partial class Form1 : Form
    {
        ...
        public void AddCoord(int amount) //создает случайные координаты заводов
        {
        ...
        {
            label2.Text = "Количество заводов (от 5 до 12):";
            label3.Text = "Количество машин (от 30 до 90), тыс:";
            button1.Text = "Ок";
            button2.Text = "Пуск";
            button3.Text = "Стоп";
            timer1.Interval = 100;
        ...
        private void DrawFactory(int x, int y) //рисует завод
        {
            Graphics gr = pictureBox1.CreateGraphics();
            gr.DrawRectangle(Pens.Black, x, y, 10, 10);
        }

        private void DrawPollutionRad(int x, int y, double pollution, double carspol) //загрязнение вокруг завода
        {

            int red = 0;
            int green = 0;
            int blue = 0;
            if (pollution + carspol >= 0 && pollution + carspol < 51) 
            {
        ...
        public void CarsPollutionColour(double pollution) //загрязнение от машин
        {
            Graphics gr = pictureBox1.CreateGraphics();
            int red = 0;
            int green = 0;
        ...
        public void Step()
        {
            ...
            foreach (Factory el in FactoryList)
            {
                ...
                if (el.isworking)
                {
                    ...
                    el.pollution = el.pollution + el.AddingPollution();
                }
            ...
            Weather.BeforeWindyDayCount++;
            if (cars.AllPollution > 200) //если загрязнение от машин превышет 200, то вводится спец режим, пока не опустится до 50
                cars.specialmode = true;
            if (cars.AllPollution < 50)
                cars.specialmode = false;
            cars.AllPollution = cars.AllPollution + cars.CountingAddingCarsPollution();
            if (k == 20)
                cars.AllPollution -= 10;
        } 
